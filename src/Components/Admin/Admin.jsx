...
const AdminPage = () => {
    ...
    Modal.confirm({
      ...
      content: (
        ...
        >
          ...
          >
            <Input.TextArea
              style={{ width: "100%", height: "70px", border: "1px solid grey" }}
          ...
          <Form.Item
            label="Warehouse URL"
            name="warehouseURL"
            rules={[{ required: true, message: "Please enter the warehouse URL" }]}
          >
            <Input
          ...
          This action will approve the order. Once approved, it cannot be undone.
        </Form>
      ),
      okText: "Yes, Approve",
      ...
      onOk: async () => {
        try {
          ...
          const pid = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/finance/getPaymentReceiptId`,{headers: {
            Authorization: `Bearer ${localStorage.getItem("admin-token")}`,
            "Content-Type": "application/json",
          }});    
  ...
  const handleAdminLogin = () => {
    ...
    axios.post(url,{email : adminemail,password:adminpassword})
    .then(res=>{
  ...
  const handleUpdateOrderStatus = (orderId) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await axios.post(
            `${process.env.REACT_APP_BACKEND_URL}/sales/updateorderstatus/${orderId}`,{headers: {
              Authorization: `Bearer ${localStorage.getItem("admin-token")}`,
              "Content-Type": "application/json",
            }}
          )
                    ...
                    {filteredAllOrders.length > 0 ? (
                      filteredAllOrders.map((order, index) => (
                        <tr key={index}>
                          ...
                          <td style={{ textAlign: "center" }}>{order.payment_status?"Paid":"Pending"}</td>
                          <td style={{ textAlign: "center" }}>{order.deliveryStatus?"Delivered":(<>{order.payment_status?(<>{order.payment_verified?"Delivery Pending":"Payment Not Approved"}</>):"Payment Pending"}</>)}</td>
